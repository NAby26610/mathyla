...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        if ($agence):
            ...
            // OPERATION =================================================================
            $Transaction = ModeleClasse::getallbyName("transactions", 'created_by', $Affectation['id_utilisateur']);
            foreach ($Transaction as $data):
                // Calcul du gains
                if ($data['statut_transaction'] == 'valider'):
                    if ($data['typeTransaction'] == 1)
                        $_SOLDE += $data['montant'];
                    elseif ($data['typeTransaction'] == 0 || $data['typeTransaction'] == -1)
                        $_SOLDE -= $data['montant'];
                endif;
            endforeach;
